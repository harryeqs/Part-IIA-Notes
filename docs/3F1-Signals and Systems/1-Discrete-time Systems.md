...
$$
\{\delta_k\}_{k \geq 0} = \{1, 0, 0, ...\} \;\text{and}\; \delta_k = 0 \;\text{for}\; k < 0
$$

- Input signal $\{u_k\}$ to an LTIS can be written as 

$$
u_k = \sum^{\infty}_{n=-\infty}\delta_nu_{k-n} \;\text{for every}\; k
$$

- By superposition principle (linearity + time invariance), the output signal $\{y_k\}$ satisfies: 

$$
y_k = \sum^{\infty}_{n = -\infty}g_nu_{k-n} \;\text{for
 every}\; k
$$
...
$$
y_k = \sum^{\infty}_{n = -\infty}g_nu_{k-n} \;\text{for every}\; k
$$
...
$$
y_k = \sum^{\infty}_{n = -\infty}g_nu_{k-n} = y_k = \sum^{\infty}_{n = -\infty}g_{k-n}u_{n}
$$

- For semi-infinite sequences $y = \{g_k\}_{k\geq0} * \{u_k\}_{k\geq0}$ simplifies to 

$$
y_k = \sum^{k}_{n = 0}g_nu_{k-n}
$$
...
$$
\{g_k\}_{k\geq0} = \{g_0, g_1, ... g_l, 0, 0, 0, ...\}
$$

- Infinite Impulse Response (IIR): 

$$
\{g_k\}_{k\geq0} = \{g_0, g_1, g_2, g_3\}
$$ 
does not terminate
...
$$
\boxed{U(z) = \sum^{\infty}_{k=0}u_kz^{-k}}
$$
...
$$
\boxed{U(\theta) = \sum^{\infty}_{k=-\infty}u_k e^{-jk\theta}}
$$

- If a signal is zero for negative values, then: 

$$
U(\theta)=U(z) \text{ for } z=e^{j \theta}, \text{ i.e. }, U(\theta)=U\left(e^{j \theta}\right)
$$
...
$$
\mathcal{Z}\left[\alpha\left\{x_k\right\}+\beta\left\{y_k\right\}\right]=\alpha \mathcal{Z}\left[\left\{x_k\right\}\right]+\beta \mathcal{Z}\left[\left\{y_k\right\}\right]
$$
...
$$
\mathcal{Z}\left[\frac{d}{d \alpha} u_k\right]=\frac{d}{d \alpha} U(z)
$$
...
$$
U^{\prime}(\theta)=\sum_{k=-\infty}^{\infty} u_{k-d} e^{-j k \theta}=\sum_{k^{\prime}=-\infty}^{\infty} u_{k^{\prime}} e^{-j\left(k^{\prime}+d\right) \theta}=e^{-j d \theta} U(\theta)
$$
...
$$
\mathcal{Z}\left[\left\{u_{k-d}\right\}\right]=z^{-d} \mathcal{Z}\left[\left\{u_k\right\}\right]
$$
...
- Similar to the Fourier transform, if a discrete-time signal $\mathbf{s}$ is real: 
$$
S(\bar{z})=\bar{S}(z)
$$
...
$$
s_k=\frac{1}{2 \pi} \int_{-\pi}^\pi S(\theta) e^{j k \theta} d \theta
$$

### 4. The DTFT vs Fourier series
- DTFT: 
$$ 
S(\theta)=\sum_{k=-\infty}^{\infty} s_k e^{-j \theta k} \quad s_k=\frac{1}{2 \pi} \int_{-\pi}^\pi S(\theta) e^{j k \theta} d \theta 
$$
