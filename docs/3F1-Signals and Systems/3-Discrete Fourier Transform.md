...
$$
 \vec{f}_n=\vec{f}_{N-n}^*, 
$$

 i.e., columns (and rows) $n$ with $n$ in range 1 to $\lceil N / 2-1\rceil$ come in complex conjugate pairs with columns $N-n$, while column 0 is real (all ones) and column $N / 2$ (for even $N$ ) is also real (alternating 1 s and -1 s ). 
...
$$ 
\left\{\begin{array}{l} \left|X_n\right|=\left|X_{N-n}\right| \\ \angle X_n=-\angle X_{N-n} \\ X_0 \text { is real } \\ \left.X_{N / 2} \text { is real (for even } N\right) \end{array}\right. 
$$

Conversely, if an input time-domain vector obeys the conjugate symmetry property, its DFT is real.
**In summary:**
...
!!! note "Circular Convolution Property"
...
$$
W_N^L=e^{-j \frac{2 \pi}{L} L}=e^{-j \frac{2 \pi}{M \cdot L} \cdot L}=e^{-j \frac{2 \pi}{M}} = W_M
$$
