...
### Setup
Consider the following scenario the system convolves its delta response with the input signal 

$$ 
\left\{Y_k\right\}=\left\{h_k\right\} \star\left\{X_k\right\} 
$$

 The input is i.i.d. (white noise) with $E\left[X_k\right]=0$ and $E\left[X_k^2\right]=1$
### Derivation
