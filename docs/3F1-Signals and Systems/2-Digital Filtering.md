...
This is known as the frequency warping 
$$
\boxed{G\left(e^{j \theta}\right)=G_c(j \tan (\theta / 2)) \quad}
$$
...
The normalised frequency of a given critical frequency $f_c$ sampled with frequency $f_s$ is 
$$
\theta = 2\pi\frac{f_c}{f_s}
$$
...
For DTFT: 
$$
w_n=u_n v_n \longrightarrow W(\theta)=\frac{1}{2 \pi} \int_{-\pi}^\pi U(\varphi) V(\theta-\varphi) d \varphi
$$
