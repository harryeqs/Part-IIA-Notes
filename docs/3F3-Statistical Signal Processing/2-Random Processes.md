...
For initial condition $\mathbf{p}^{(0)}$ and transition matrix $Q$, we have

$$
\mathbf{p}^{(n)}=P\left(X_n\right)=\mathbf{p}^{(0)} Q^n
$$
...
### 5. Ergodicity
Let $\mathbf{x}=\left(x_1, x_2, \ldots\right)$ be a sample from $\left(X_1, X_1, \ldots\right)$ and let $f(x)$ be some function of the states.
